...
from transformers import (
    ...
    RobertaTokenizerFast, RobertaModel, RobertaConfig, RobertaPreTrainedModel, 
...
BASE_MODEL_NAME = model #"bert-base-multilingual-cased"
MODEL_DIR = os.path.join(DATA_DIR, "{:s}-ct-ebm-sp-v2".format(BASE_MODEL_NAME))
...
class RobertaForRelationExtraction(PreTrainedModel):
  def __init__(self, config, num_labels):
    super(RobertaForRelationExtraction, self).__init__(config)
...
model.roberta.resize_token_embeddings(len(tokenizer.vocab))
...
with open(BASE_MODEL_NAME.replace('/','-')+"_results_v2.txt",mode="a") as f:
...
def plot_confusion_matrix(ytrue, ypreds, labels,iteration):
  ...
  plt.savefig(BASE_MODEL_NAME.replace('/','-')+"-"+str(iteration)+"_v2.png")
  _ = plt.show()
